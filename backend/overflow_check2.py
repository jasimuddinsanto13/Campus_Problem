...
class CDP:
    def __init__(self, ws_url): self.ws_url = ws_url; self.msg_id = 0
    async def connect(self): self.ws = await websockets.connect(self.ws_url, max_size=10_000_000)
    ...
    async def navigate(self, url): await self.send('Page.navigate', {'url': url}); await asyncio.sleep(3)
...
async def main():
    profile = tempfile.mkdtemp(prefix='ovf2-')
    ...
    await cdp.evaljs("document.querySelector('input[value=teacher]').click()")
    await cdp.type_selector('input[name=email]', 'teacher')
    await cdp.type_selector('input[name=password]', 'demo1234')
    await cdp.click_selector('button[type=submit]')
    await asyncio.sleep(3)
    await cdp.set_viewport(420, 1100)
    await cdp.navigate(BASE + '/portal/faculty/')
    await asyncio.sleep(2)
    print('FACULTY page — doc width:', await cdp.evaljs('document.documentElement.scrollWidth'), 'client:', await cdp.evaljs('document.documentElement.clientWidth'))
    print('main-content rect:', await cdp.evaljs("[Math.round(document.querySelector('.main-content').getBoundingClientRect().left), Math.round(document.querySelector('.main-content').getBoundingClientRect().right)]"))
    proc.terminate()
